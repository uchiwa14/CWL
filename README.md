...
2. GUI が表示されたら、`業務開始` をクリックします。
   - 指定したプロファイルの Chrome を起動します。
  - すでに業務 URL を開いている対象プロファイルのウィンドウがある場合は再利用し、設定した位置とサイズで配置します。
3. 作業が終わったら、`業務終了` をクリックします。
   - 本ツールで起動・自動配置された管理対象のウィンドウ一覧を表示する確認ダイアログが出ます。承認するとまとめて閉じます。
...
- 起動時に設定ファイルの厳密なチェックを行います。指定された `Count` 数に合わない不足・余剰セクション、あるいは不正な値が含まれる場合、エラーダイアログを表示して起動を停止します。
- `ChromePath` を指定する場合は、`chrome.exe` まで含めたパスを入力してください。
- 本ツールは AutoHotkey v2 用です。v1 では動作しません。

## 連絡先
- リポジトリ管理者に問い合わせる場合は、README の配布元または `doc/CWL_仕様書.md` / `doc/CWL_設計書.md` を確認してください。
